...
// Exported functions
void aprs_send(int packetNumber)
{
	...
	// Humidity: "/Rh=84.56"
	ax25_send_string("/Rh=");
	dtostrf(sensors_humidity(), -1, 0, temp);
	ax25_send_string(temp);
    DEBUG_UART.print(temp);
    DEBUG_UART.print(",");
  ...
  ax25_send_string("/Te=");     // TODO replace with external temp
  dtostrf(sensors_extTemperature(), -1, 1, temp);
  ax25_send_string(temp);
    DEBUG_UART.print(temp);
    DEBUG_UART.print(",");
  
	ax25_send_string("/Vb=");
	dtostrf(sensors_battery(), -1, 1, temp);
	ax25_send_string(temp);
    DEBUG_UART.print(temp);
    DEBUG_UART.println("");

  ax25_send_string("/i=");
  snprintf(temp, 6, "%d", packetNumber);
  ax25_send_string(temp);
